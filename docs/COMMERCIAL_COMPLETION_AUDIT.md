...
- production version: `0.10.0-rc25`
- schema: `25`
- runtime commit: `8eba942a493b617ecd80417de330409a514b5a5a`
...
| Customer self-service and commercial admin UI | Login/register/reset/verification, multi-tenant selector, session/security/privacy centers, keys, balance, usage, members, designated-Owner display/MFA transfer and Checkout; tenant/price/order/refund/dispute/admin controls | Complete in code/dark launch; real multi-tenant selector/ownership QA pending |
...
| Production deployment and acceptance | HTTPS runtime reports exact release/schema; schema25, Scheduler, privacy/security/credential/multi-tenant session and designated-ownership lifecycle, alert/email Outboxes and immutable legal/privacy evidence; public legal metadata remains unconfigured/unapproved; hard gates and zero unintended tenant/membership/ownership/session/financial/legal/privacy rows verified | Dark launch complete; public charging deliberately disabled |
...
- restored accounts: 5;
- restored customer sessions, memberships, ownership rows and pending MFA candidates: 0 / 0 / 0 / 0;
- restored plans: 2; plan periods: 0;
- `information_schema` trigger rows: 25;
...
13. authenticated staging acceptance for designated-Owner transfer between two

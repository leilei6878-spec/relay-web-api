...
test("backup/restore: PGlite dump JSON snapshot into a new database and verify rows", async () => {
  ...
  assert.equal(ver.rows[0].value, "15");
...
test("restore dry-run verifies a database archive without requiring a destination database", (t) => {
  ...
  const dryRun = runScript("restore.mjs", ["--from", dir, "--storage", join(dir, "unused"), "--dry-run"]);
  assert.equal(dryRun.status, 0, dryRun.stderr || dryRun.stdout);
  assert.equal(JSON.parse(dryRun.stdout).database, "dumped");
  const actual = runScript("restore.mjs", ["--from", dir, "--storage", join(dir, "must-not-exist")]);
...
test("restore rejects a symlinked parent that escapes the backup storage root", (t) => {
  ...
  const rejected = runScript("restore.mjs", ["--from", backup, "--storage", join(dir, "restore"), "--dry-run"]);
  assert.notEqual(rejected.status, 0);

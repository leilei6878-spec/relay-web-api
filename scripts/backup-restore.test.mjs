...
test("backup/restore: PGlite dump JSON snapshot into a new database and verify rows", async () => {
  const pg = new PGlite();
  await pg.waitReady;
  for (const name of ["0001_relay.sql", "0002_relay_ops.sql", "0003_relay_production.sql", "0004_schema_meta.sql", "0005_account_operations.sql"]) {
  ...
  assert.equal(ver.rows[0].value, "5");

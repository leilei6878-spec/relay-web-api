...
export function SaasMfaDialog() {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="mt-4" variant="secondary">配置 MFA</Button></DialogTrigger><DialogContent title="TOTP 多因素认证">{recovery.length ? <div><p className="text-sm text-warn">请离线保存以下恢复码，每个恢复码只能使用一次。</p><pre className="mt-3 rounded-md bg-elevated p-3 text-xs">{recovery.join("\n")}</pre><Button className="mt-4 w-full" onClick={() => window.location.reload()}>已安全保存，刷新会话状态</Button></div> : secret ? <div className="space-y-3"><p className="text-sm text-muted">在身份验证器中手动添加密钥：</p><p className="break-all rounded-md bg-elevated p-3 font-mono text-xs">{secret}</p><div className="space-y-2"><Label>当前 6 位验证码</Label><Input inputMode="numeric" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ""))} /></div><Button className="w-full" onClick={() => void confirm()}>验证并启用</Button></div> : <div><p className="text-sm text-muted">启用后登录必须同时输入密码和身份验证器验证码。</p><Button className="mt-4 w-full" onClick={() => void start()}>生成 TOTP 密钥</Button></div>}</DialogContent></Dialog>;
}

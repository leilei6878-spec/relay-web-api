...
function PasswordChangeDialog({ onChanged }: { onChanged: () => Promise<void> }) {
  ...
  async function submit() {
    ...
    setBusy(true);
    try {
      const response = await fetch("/api/saas/security", {
        method: "POST", credentials: "include", headers: saasMutationHeaders(),
        body: JSON.stringify({ action: "change-password", currentPassword, newPassword }),
      });
      const body = await response.json() as { ok?: boolean; error?: string; result?: { revokedSessions?: number } };
      if (!response.ok || !body.ok) throw new Error(body.error || "密码修改失败");
      toast.success(`密码已修改，已撤销 ${body.result?.revokedSessions || 0} 个其他会话`);
      setOpen(false); await onChanged();
    } catch (error) { toast.error(error instanceof Error ? error.message : "密码修改失败"); }
    finally { setBusy(false); }
  }
  return <Dialog open={open} onOpenChange={(value) => { setOpen(value); if (!value) { setCurrentPassword(""); setNewPassword(""); setConfirmation(""); } }}><DialogTrigger asChild><Button size="sm" variant="secondary">修改密码</Button></DialogTrigger><DialogContent title="安全修改密码"><div className="space-y-3"><p className="text-xs text-muted">需要验证当前密码；启用 MFA 后还要求当前会话近期通过 MFA。成功后保留当前设备并撤销其他设备。</p><div className="space-y-2"><Label>当前密码</Label><Input type="password" autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} /></div><div className="space-y-2"><Label>新密码（10–1024 个字符）</Label><Input type="password" autoComplete="new-password" minLength={10} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} /></div><div className="space-y-2"><Label>再次输入新密码</Label><Input type="password" autoComplete="new-password" minLength={10} value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></div><Button className="w-full" disabled={busy || currentPassword.length<1 || newPassword.length<10 || newPassword!==confirmation} onClick={() => void submit()}>{busy ? "正在修改…" : "修改并撤销其他会话"}</Button></div></DialogContent></Dialog>;
}

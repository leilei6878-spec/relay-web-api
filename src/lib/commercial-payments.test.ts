...
test("refund reserves credit before the provider call and writes one balanced reversal", async () => {
  const { pg, db, owner } = await database();
  let orderId = "";
  const checkoutFetch = (async (_url: string | URL | Request, init?: RequestInit) => {
    orderId = new URLSearchParams(String(init?.body)).get("metadata[order_id]")!;
  ...
  await assert.rejects(
    () => createStripeRefund({ orderId, amountMinor: 1000, reason: "unsafe partial tax refund", idempotencyKey: "refund-tax-partial", actor: "admin" }, { env, db, fetcher: checkoutFetch }),
    /PARTIAL_TAX_REFUND_REQUIRES_STRIPE_TAX_REVERSAL/,
  );
  const refundFetch = (async (_url: string | URL | Request, init?: RequestInit) => {
    const held = await pg.query<{ reserved_minor: bigint }>("select reserved_minor from relay_tenants where id=$1", [owner.tenantId]);
    assert.equal(Number(held.rows[0]?.reserved_minor), 2500);
    const form = new URLSearchParams(String(init?.body));
    assert.equal(form.get("payment_intent"), "pi_refundable");
    assert.equal(form.get("amount"), "2750");
    return Response.json({ id: "re_test_refund_1", amount: 2750, status: "succeeded" });
  }) as typeof fetch;
  const result = await createStripeRefund({ orderId, amountMinor: 2500, reason: "QA refund", idempotencyKey: "refund-1", actor: "admin" }, { env, db, fetcher: refundFetch });
  assert.equal(result.replay, false);
  const tenant = await pg.query<{ balance_minor: bigint; reserved_minor: bigint }>("select balance_minor,reserved_minor from relay_tenants where id=$1", [owner.tenantId]);
  assert.equal(Number(tenant.rows[0]?.balance_minor), 0);
  assert.equal(Number(tenant.rows[0]?.reserved_minor), 0);
  const order = await pg.query<{ status: string; refunded_minor: bigint; refunded_tax_minor: bigint; refunded_gross_minor: bigint }>("select status,refunded_minor,refunded_tax_minor,refunded_gross_minor from relay_orders where id=$1", [orderId]);
  ...
  const replay = await createStripeRefund({ orderId, amountMinor: 2500, reason: "QA refund", idempotencyKey: "refund-1", actor: "admin" }, { env, db, fetcher: refundFetch });
  assert.equal(replay.replay, true);
  await pg.close();
});

...
test("image API routes ChatGPT LLM image through ChatGPT accounts and the image-safe worker path", async () => {
  ...
  assert.match(worker, /def download_chatgpt_image_action\(page, boundary=None\)/);
  assert.match(worker, /page\.expect_download/);
  ...
  assert.match(worker, /isNewSrc.*resultAction.*w >= 512 and h >= 512/);

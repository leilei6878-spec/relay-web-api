...
export async function commercialAdminSnapshot(db?: DbLike) {
  const sql = await database(db);
  const [tenants, plans, planPeriods, prices, orders, transactions, alerts, paymentEvents, refunds, disputes, tenantAudits, emailDeliveries, legalAcceptances, privacyRequests] = await Promise.all([
    ...
    sql.query<Record<string, unknown>>(
      `select a.*,
              (select d.event_type from relay_alert_deliveries d where d.alert_id=a.id order by d.created_at desc limit 1) as delivery_event_type,
              (select d.status from relay_alert_deliveries d where d.alert_id=a.id order by d.created_at desc limit 1) as delivery_status,
              coalesce((select sum(d.attempts) from relay_alert_deliveries d where d.alert_id=a.id),0)::int as delivery_attempts,
              (select d.last_attempt_at from relay_alert_deliveries d where d.alert_id=a.id order by d.created_at desc limit 1) as last_delivery_at,
              (select d.next_attempt_at from relay_alert_deliveries d where d.alert_id=a.id order by d.created_at desc limit 1) as next_delivery_at,
              (select d.delivered_at from relay_alert_deliveries d where d.alert_id=a.id order by d.created_at desc limit 1) as delivered_at,
              (select d.http_status from relay_alert_deliveries d where d.alert_id=a.id order by d.created_at desc limit 1) as delivery_http_status,
              (select d.error_code from relay_alert_deliveries d where d.alert_id=a.id order by d.created_at desc limit 1) as delivery_error_code
         from relay_alert_events a order by a.last_seen_at desc limit 200`,
    ),
    ...
    sql.query<Record<string, unknown>>(
      `select id,dedupe_key,kind,status,attempts,recipient_hmac,last_attempt_at,next_attempt_at,
    ...
    sql.query<Record<string, unknown>>(
      `select id,tenant_id,requested_by,kind,status,due_at,snapshot_sha256,blocked_reason,
         ...
         from relay_privacy_requests order by requested_at desc,id desc limit 500`,
    ),
  ]);
  return { tenants, plans, planPeriods, prices, orders, transactions, alerts, paymentEvents, refunds, disputes, tenantAudits, emailDeliveries, legalAcceptances, privacyRequests };
}

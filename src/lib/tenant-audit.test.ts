...
const migrations = [
  ...
  "0014_saas_session_mfa.sql", "0015_tenant_audit.sql", "0016_alert_delivery_outbox.sql", "0017_email_delivery_outbox.sql", "0018_legal_acceptance.sql", "0019_legal_reconsent.sql", "0020_tenant_privacy_rights.sql",
];
...
test("tenant audit route and all privileged tenant mutation routes keep the audit invariant", async () => {
  ...
  for (const path of ["keys.ts", "members.ts", "billing.ts", "session.ts"]) {

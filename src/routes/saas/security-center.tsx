...
function SecurityCenter() {
  ...
  return <SaasShell tenant={session.tenant}><div className="space-y-5"><header><p className="text-xs text-muted">独立安全入口</p><h1 className="mt-2 text-2xl font-semibold">账户安全中心</h1><p className="mt-2 text-sm text-muted">即使租户暂停或法律文件等待重新同意，你仍可检查设备、撤销会话和保护 MFA。</p></header>{session.legalAcceptanceRequired ? <div className="rounded-xl border border-warn/30 bg-warn/5 p-4 text-sm text-warn">当前法律文件尚未重新同意；安全中心保持可用，普通服务 API 仍受限。</div> : null}<section className="rounded-xl border border-border bg-surface p-5"><h2 className="font-medium">{session.user.mfaEnabled ? "验证器管理" : "启用多因素认证"}</h2><p className="mt-2 text-sm text-muted">{session.user.mfaEnabled ? "更换验证器会先暂存新 Secret；确认成功前旧验证器持续生效。" : "Owner/Admin 必须在商业开放前启用 TOTP。启用操作不以接受新版条款为条件。"}</p><SaasMfaDialog mfaEnabled={session.user.mfaEnabled} /></section><SaasSessionSecurity mfaEnabled={session.user.mfaEnabled} /></div></SaasShell>;
}
